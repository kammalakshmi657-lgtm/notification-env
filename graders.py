...
def grade_task1(action, ground_truth):
    ...
    if total == 0:
        return Reward(score=0.0, feedback="No notifications to grade.")
    exact_correct = 0
    partial_correct = 0
    per_item = {}
    for label in action.labels:
        ...
        if predicted == expected:
            ...
            per_item[nid] = 0.0
    score = round((exact_correct + 0.5 * partial_correct) / total, 4)
...
def grade_task2(action, ground_truth_rank):
    ...
    if not agent_labels:
        return Reward(score=0.0, feedback="No priority ranks provided.")
    agent_sorted = sorted(agent_labels.items(), key=lambda x: x[1])
    agent_ranking = [item[0] for item in agent_sorted]
    score = _kendall_tau(gt_ranking, agent_ranking)
...
def grade_task3(action, ground_truth):
    ...
    for label in action.labels:
        ...
        if predicted_action is None:
            action_scores[nid] = 0.0
            continue
        if predicted_action == expected_action:
            action_scores[nid] = 1.0
        else:
            dist = abs(action_adjacency[predicted_action] - action_adjacency[expected_action])
            action_scores[nid] = max(0.0, 1.0 - dist * 0.4)
        if ground_truth[nid]["needs_summary"]:
            summary_scores[nid] = _score_summary(label.summary or "")
    avg_action = sum(action_scores.values()) / total if action_scores else 0.0
    avg_summary = sum(summary_scores.get(nid, 0.0) for nid in escalated_expected) / len(escalated_expected) if escalated_expected else 1.0
    score = round(0.7 * avg_action + 0.3 * avg_summary, 4)
